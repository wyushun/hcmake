...
typedef struct {
    ...
    uint32_t profiler_en;  /*[Input] Specify the core profiler function to be used*/
    uint32_t reg_count;    /*[Input] Specify the count of registers to be written. No more than NPU_AOL_REG_NUM. */
    npu_aol_reg_t regs[NPU_AOL_REG_NUM]; /*[Input] The registers data buffer to be written. The actual count is specified by reg_count. */
} npu_aol_run_t;

typedef struct {
    uint32_t reg_count;    /*[Input] Specify the count of registers to be written. No more than NPU_AOL_REG_NUM. */
    npu_aol_reg_t regs[NPU_AOL_REG_NUM];     /*[Input] The registers data buffer to be written. The actual count is specified by reg_count. */
    uint32_t regs_delay_us[NPU_AOL_REG_NUM]; /*[Input] The delay time array in microsecond after writing each register specified by regs. */
} npu_aol_init_t;


struct req_mem_alloc_t {
    unsigned long       size;        /* size of memory space to be allocated */
    unsigned long       addr_phy;    /* suv the start pyhsical address of allocated NPU memory (RETURNED) */
};

struct req_mem_free_t {
    unsigned long       addr_phy;    /* the start pyhsical address of allocated NPU memory */
};
 ...
 * Input:
 *     mode - Conventional the NPU scheduling mode. Only NPU_SCHEDULE_MODE_SINGLE used currently.
 * Return:
 ...
 */
int npu_aol_read_regs(npu_aol_dev_handle_t *dev, uint64_t phy_address, uint32_t *buf, uint32_t count);

/* Initialize NPU or other IPs. It may be called when the IP first starts or times out.
 * Input:
 *     dev - The device handle obtained by npu_aol_attach.
 * Input/Output:
 *     data - The data required for this scheduling. See npu_aol_init_t for detail.
 * Return:
 *     NPU_AOL_OK for success, NPU_AOL_ERROR for failure.
 */
 ...
 *     dev - The device handle obtained by npu_aol_attach.
 * Input/Output:
 *     data - The data required for this scheduling. See npu_aol_run_t for detail.
 * Return:
 *     NPU_AOL_OK for success, NPU_AOL_ERROR for timeout.
 */
 ...
 *     prot - Fxied to (NPU_AOL_MEM_PROT_READ|NPU_AOL_MEM_PROT_WRITE) in this version.
 * Return:
...
npu_aol_dev_mem_t *npu_aol_alloc_dev_mem(npu_aol_dev_handle_t *dev, uint64_t size, uint32_t prot);
 ...
 *     NPU_AOL_OK for success, NPU_AOL_ERROR for failure.
 */
int npu_aol_sync_to_dev(npu_aol_dev_handle_t *dev, const npu_aol_dev_mem_t *mem, uint32_t offset, uint32_t size);
 ...
 *     NPU_AOL_OK for success, NPU_AOL_ERROR for failure.
 */
int npu_aol_sync_from_dev(npu_aol_dev_handle_t *dev, npu_aol_dev_mem_t *mem, uint32_t offset, uint32_t size);
